...
function displayResults() {
    ...
    if (specialization && location) {
        ...
        if (headerElement) headerElement.textContent = `We found the following doctors in ${location} specializing in ${specialization}${date ? ` on ${date}` : ''}.`;
        else console.warn("Header element with class '.we-found-the' not found.");
        // Filter doctors based on specialization and location
        let filteredDoctors = doctors.filter((doctor)=>doctor.specialization.toLowerCase() === specialization.toLowerCase() && doctor.location.toLowerCase() === location.toLowerCase() && (!date || doctor.availableDates.includes(date)));
        ...
        const renderDoctors = (doctorsToRender)=>{
            ...
            `;
                ...
                } else doctorResults.innerHTML = `<p>No doctors found for the selected criteria.</p>`;
            } else console.warn("Doctor results element not found.");
        };
        // Initial render
    ...
    } else console.warn("Specialization or location not provided in query params.");
}
// Run on page load
document.addEventListener("DOMContentLoaded", displayResults);

//# sourceMappingURL=result.96b28112.js.map
